...
model.materials = openmc.Materials([uo2_low,uo2_high, zirconium, water,helium,pyrex_mat,SS304]) 
...
def low_pincell():
    ...
    fuel = openmc.Cell(name='low_fuel')
    fuel.region = fuel_region
    fuel.fill = uo2_low
    #new_fuel = uo2.clone()
    #fuel.fill = new_fuel

    gap = openmc.Cell(name='low_fuel_gap')
    gap.region = gap_region
    gap.fill = helium     

    clad = openmc.Cell(name='low_fuel_clad')
    clad.region = clad_region
    clad.fill = zirconium

    moderator = openmc.Cell(name='low_fuel_moderator')
    moderator.region = water_region
    moderator.fill = water

    return openmc.Universe(name='low-pin-univ', cells=(fuel, clad, moderator,gap))
...
def high_pincell():
    ...
    fuel = openmc.Cell(name='high_fuel')
    fuel.region = fuel_region
    fuel.fill = uo2_high
    #new_fuel = uo2.clone()
    #fuel.fill = new_fuel

    gap = openmc.Cell(name='high_fuel_gap')
    gap.region = gap_region
    gap.fill = helium     

    clad = openmc.Cell(name='high_fuel_clad')
    clad.region = clad_region
    clad.fill = zirconium

    moderator = openmc.Cell(name='high_fuel_moderator')
    moderator.region = water_region
    moderator.fill = water

    return openmc.Universe(name='high-pin-univ', cells=(fuel, clad, moderator,gap))
...
pcl_univ  = low_pincell()
pch_univ = high_pincell()
it_univ  = instrument()
py_univ = pyrex_pin()
...
for row in range(0,lattice_csv.shape[0]):
    for col in range(0,lattice_csv.shape[1]):
        ...
        elif lattice_csv[row][col] == 'ful':
            lattice_array[row][col] = pcl_univ
        elif lattice_csv[row][col] == 'fuh':
            lattice_array[row][col] = pch_univ
