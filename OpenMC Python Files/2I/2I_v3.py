...
model.materials = openmc.Materials([uo2, zirconium, water,helium,SS304]) 
...
# Thimble radii
th_inner_radius = openmc.ZCylinder(r=0.258)
th_outer_radius = openmc.ZCylinder(r=0.382)
...
def instrument():
    ...
    th_region = +th_inner_radius & -th_outer_radius 
    it_water_region = +left & -right & +bottom & -top & +it_outer_radius | +th_outer_radius & -it_inner_radius
    it_region = +it_inner_radius & -it_outer_radius
    it_gap_region = -th_inner_radius

    water_cell = openmc.Cell(name='it-water')
    water_cell.fill = water
    water_cell.region = it_water_region

    th_cell = openmc.Cell(name='it-clad')
    th_cell.fill = SS304
    th_cell.region = th_region

    it_cell = openmc.Cell(name='instrument')
    it_cell.fill = zirconium
    it_cell.region = it_region

    it_gap = openmc.Cell(name='it_gap')
    it_gap.region = it_gap_region
    it_gap.fill = helium 

    return openmc.Universe(name='it-univ', cells=(water_cell, it_cell, th_cell,it_gap))
