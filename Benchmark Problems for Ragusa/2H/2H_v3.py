...
model.materials = openmc.Materials([uo2, zirconium, water,helium,B4C_mat,SS304]) 
...
#B4C
b4c_radius= openmc.ZCylinder(r= 0.373)
b4c_clad_inner_radius= openmc.ZCylinder(r= 0.386)
b4c_clad_outer_radius= openmc.ZCylinder(r= 0.484)
...
def b4c():
    ...
    b4c_region= -b4c_radius
    water_region = +left & -right & +bottom & -top & +gt_outer_radius | +b4c_clad_outer_radius & -gt_inner_radius
    ...
    return openmc.Universe(name='b4c-univ', cells=(steel_cell,gap_cell,b4c_cell,water_cell,guide_cell))
...
for row in range(0,lattice_csv.shape[0]):
    for col in range(0,lattice_csv.shape[1]):
        ...
        elif lattice_csv[row][col] == 'b':
            lattice_array[row][col] = b4c_univ
        else:
