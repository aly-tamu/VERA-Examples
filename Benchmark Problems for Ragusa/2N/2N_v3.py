...
poison_mat = openmc.Material(name="poison_mat")

poison_mat.add_nuclide('B10',2.98553E-03, 'ao')
poison_mat.add_nuclide('B11',1.21192E-02, 'ao')
poison_mat.add_nuclide('C0',3.77001E-03, 'ao')
poison_mat.add_nuclide('O16',5.85563E-02, 'ao')
poison_mat.add_nuclide('Al27',3.90223E-02, 'ao')

poison_mat.temperature=600

poison_mat.set_density('g/cm3', 3.65)
...
model.materials = openmc.Materials([uo2, zirconium, water,helium,coat_mat,poison_mat]) 
...
#WABA
Inner_Tube_Inner_Radius= openmc.ZCylinder(r=0.286)
Inner_Tube_Outer_Radius= openmc.ZCylinder(r= 0.339)
Poison_Inner_Radius= openmc.ZCylinder(r= 0.353)
Poison_Outer_Radius= openmc.ZCylinder(r= 0.404)
waba_clad_outer_radius= openmc.ZCylinder(r= 0.484)
...
def waba_pin():
    ...
    poison_region= +Poison_Inner_Radius & -Poison_Outer_Radius
    waba_water_region = +left & -right & +bottom & -top & +gt_outer_radius | +waba_clad_outer_radius & -gt_inner_radius| -Inner_Tube_Inner_Radius
    waba_guide_region = +gt_inner_radius & -gt_outer_radius
    waba_clad_region = +clad_inner_radius & -waba_clad_outer_radius | +Inner_Tube_Inner_Radius & -Inner_Tube_Outer_Radius
    waba_gap_region = +Inner_Tube_Outer_Radius & -Poison_Inner_Radius |  +Poison_Outer_Radius & - clad_inner_radius 

    waba_clad_cell = openmc.Cell(name='waba_clad')
    waba_clad_cell.fill = zirconium
    waba_clad_cell.region = waba_clad_region    

    waba_gap_cell = openmc.Cell(name='waba_gap')
    waba_gap_cell.region = waba_gap_region 
    waba_gap_cell.fill = helium

    poison_cell = openmc.Cell(name='poison')
    poison_cell.fill = poison_mat
    poison_cell.region = poison_region 

    waba_water_cell = openmc.Cell(name='waba_water')
    waba_water_cell.fill = water
    waba_water_cell.region = waba_water_region

    waba_guide_cell = openmc.Cell(name='waba_guide')
    waba_guide_cell.fill = zirconium
    waba_guide_cell.region = waba_guide_region


    return openmc.Universe(name='waba-univ', cells=(waba_guide_cell, waba_water_cell, poison_cell,waba_gap_cell,waba_clad_cell))
...
for row in range(0,lattice_csv.shape[0]):
    for col in range(0,lattice_csv.shape[1]):
        ...
        elif lattice_csv[row][col] == 'w':
            lattice_array[row][col] = waba_univ 
        else:
