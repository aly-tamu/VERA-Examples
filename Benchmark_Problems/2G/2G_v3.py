...
model.materials = openmc.Materials([uo2, zirconium, water,helium,AIC_mat,SS304]) 
...
#AIC
aic_radius= openmc.ZCylinder(r= 0.382)
aic_clad_inner_radius= openmc.ZCylinder(r= 0.386)
aic_clad_outer_radius= openmc.ZCylinder(r= 0.484)
...
def aic():
    ...
    aic_region= -aic_radius
    water_region = +left & -right & +bottom & -top & +gt_outer_radius | +aic_clad_outer_radius & -gt_inner_radius
    ...
    return openmc.Universe(name='aic-univ', cells=(steel_cell,gap_cell,aic_cell,water_cell,guide_cell))
...
for row in range(0,lattice_csv.shape[0]):
    for col in range(0,lattice_csv.shape[1]):
        ...
        elif lattice_csv[row][col] == 'a':
            lattice_array[row][col] = aic_univ
        else:
