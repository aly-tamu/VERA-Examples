    ...
    for j in range(num_cells):
        if lattice_csv[i, j] == "fu":
            ...
            for g in range(0, num_groups):
                ...
                val += val_g * sig_f[g]
            val_table[i, j] = val
        elif lattice_csv[i, j] == "gd":
            fuel_xs = xs_dict["gd_fuel"]
            sig_f = np.array(fuel_xs.sigma_f)
            x_center, y_center = compute_cell_center(i, j, offset_x, offset_y)
            if rank == 0:
                print("centers=", i, j, x_center, y_center)
...
norm = np.sum(val_table) / ( cell_frequencies["fu"] + cell_frequencies["gd"] )
val_table /= norm
