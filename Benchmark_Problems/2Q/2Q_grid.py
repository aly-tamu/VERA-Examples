...
# # Fuel assembly: 2Q with discrete grid
...
model.materials = openmc.Materials([uo2, zirconium,helium,water]) 
...
#Find the grid volume
grid_mass = 875/289
grid_height=3.810
grid_vol=grid_mass/zirconium.density

#Relate grid volume to grid thickness (for a single cell). V = (pitch^2)*h  -   ((pitch - 2*grid thickness)^2)*h
grid_width=(((grid_height*pitch)-np.sqrt(grid_height*(grid_height*(pitch**2)-grid_vol)))/(2*grid_height))

#Define grid_bounds within the cell
grid_left   = openmc.XPlane((-pitch/2) + grid_width, boundary_type='transmission')
grid_right  = openmc.XPlane( (pitch/2) - grid_width, boundary_type='transmission')
grid_bottom = openmc.YPlane( (-pitch/2)+ grid_width, boundary_type='transmission')
grid_top    = openmc.YPlane( (pitch/2) - grid_width, boundary_type='transmission')
...
def pincell():
    ...
    water_region = +grid_left & -grid_right & +grid_bottom & -grid_top & +clad_outer_radius
    grid_region = (-grid_left | +grid_right | -grid_bottom | +grid_top) & +left & -right & +bottom & -top
    ...
    grid = openmc.Cell(name='grid')
    grid.region=grid_region
    grid.fill=zirconium

    return openmc.Universe(name='pin-univ', cells=(fuel, clad, moderator,gap,grid))
...
def guide():
    ...
    gt_water_out_region = +grid_left & -grid_right & +grid_bottom & -grid_top & +gt_outer_radius
    gt_grid_region = (-grid_left | +grid_right | -grid_bottom | +grid_top) & +left & -right & +bottom & -top
    ...
    gt_grid = openmc.Cell(name='gt_grid')
    gt_grid.region=gt_grid_region
    gt_grid.fill=zirconium

    return openmc.Universe(name='gt-univ', cells=(gt_water_in, gt_clad, gt_water_out,gt_grid))
...
def instrument():
    ...
    it_water_out_region = +grid_left & -grid_right & +grid_bottom & -grid_top & +it_outer_radius
    it_grid_region = (-grid_left | +grid_right | -grid_bottom | +grid_top) & +left & -right & +bottom & -top
    ...
    it_grid = openmc.Cell(name='it_grid')
    it_grid.region=it_grid_region
    it_grid.fill=zirconium
    return openmc.Universe(name='it-univ', cells=(it_water_in, it_clad, it_water_out,it_grid))
